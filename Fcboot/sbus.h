...
#define SBUS_FREQUENCY_THRESHHOLD 100
...
void SBUS_init(UART_HandleTypeDef *huart) {
	...
	sbus.state = SBUS_STATE_LOW_FREQUENCY;
...
void SBUS_decoding() {
	...
	/* handle sbus state */
	if(sbus.hz > SBUS_FREQUENCY_THRESHHOLD){
	...
	else sbus.state = SBUS_STATE_LOW_FREQUENCY;


	if(sbus.rxBuf[23] & SBUS_LOST_FRAME) return; /* if this frame is lost frame, not decode packet */
...
void SBUS_calHz(){ // call this func 1hz
	sbus.hz = sbus.hzCnt;
	sbus.hzCnt = 0;
}
